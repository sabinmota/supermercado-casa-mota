...
/**
 * Renderiza el paginador en el elemento con id = paginatorId.
 * Solo se muestra cuando hay más de PAGE_SIZE filas.
 * @param {string} paginatorId  – id del div contenedor
 * @param {number} currentPage  – página actual (1-based)
 * @param {number} totalItems   – total de ítems filtrados
 * @param {string} section      – clave en _pages ('products','orders','inventory','customers','staff')
 * @param {string} scrollToId   – id del elemento al que hacer scroll al cambiar página
 */
function _renderPaginator(paginatorId, currentPage, totalItems, section, scrollToId) {
  ...
  // Números visibles: siempre 1, última, y ±2 alrededor de la actual
...
function _goPage(section, scrollToId, page) {
  // Calcular total filtrado en cada sección para validar límites
  const totals = {
    products: (() => {
...
async function initAdminData() {
  ...
  // Renderizar todo una vez los datos estén disponibles
  loadDashboard();
  renderProductsTable();
  renderOrdersTable();
  renderInventory();
...
async function loadDashboard() {
  // Recargar productos y pedidos frescos desde la API
  try {
    const [prods, ords] = await Promise.all([ DB.getProducts(), DB.getOrders() ]);
    adminProducts = prods.length > 0 ? prods : deepClone(PRODUCTS);
    orders        = ords;
  } catch(e) { /* usa estado actual en memoria */ }
  ...
  document.getElementById('kpiSales').textContent    = `RD$ ${totalSales.toLocaleString('es-DO')}`;
  document.getElementById('kpiOrders').textContent   = orders.length;
  document.getElementById('kpiProducts').textContent = adminProducts.length;
...
function renderSalesChart() {
  ...
  adminProducts.forEach(p => {
    catTotals[catLabel(p.category)] = (catTotals[catLabel(p.category)] || 0) + p.price * Math.floor(Math.random()*20+5);
  });
  ...
  salesChartInstance = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: { callbacks: { label: ctx => ` RD$ ${ctx.parsed.toLocaleString('es-DO')}` } }
      }
    }
  });
}
...
function renderProductsTable() {
  ...
  const filtered = adminProducts.filter(p => {
    const matchQ   = !q      || p.name.toLowerCase().includes(q) || (p.description||'').toLowerCase().includes(q) || (p.barcode||'').toLowerCase().includes(q);
  ...
  list.forEach(p => {
    ...
    tr.innerHTML = `
      ...
      <td><strong>RD$ ${p.price}</strong>${p.originalPrice?`<br><small style="text-decoration:line-through;color:#aaa">RD$ ${p.originalPrice}</small>`:''}</td>
      <td><span class="${stockClass}">${p.stock}</span></td>
      <td>${badgeHTML}</td>
      <td>⭐ ${p.rating} <small style="color:#aaa">(${p.reviews})</small></td>
      <td>${p.barcode?`<span style="font-family:monospace;font-size:.8rem;background:#f4f4f4;padding:2px 6px;border-radius:4px"><i class="fas fa-barcode" style="color:#666;margin-right:3px"></i>${p.barcode}</span>`:`<span style="color:#ddd;font-size:.78rem">—</span>`}</td>
  ...
  const productRowsHTML = lines.length > 0
    ? lines.map((l, idx) => {
        ...
        if (isPending) {
          ...
          // Badge sustitución: solo mostrar si el campo existe en el pedido (evita "No sustituir" en pedidos viejos sin el campo)
          const hasSustField = 'sustitucion' in l;
          const sustBadge = !hasSustField ? '' : l.sustitucion
            ? `<span class="sust-badge sust-badge--yes" title="Cliente autorizó sustituir este producto"><i class="fas fa-shuffle"></i> Sustituible</span>`
            : `<span class="sust-badge sust-badge--no" title="Cliente NO autorizó sustitución"><i class="fas fa-ban"></i> No sustituir</span>`;
          return `
              ...
              <td>
                <div style="display:flex;align-items:center;gap:10px">
                  ...
                  <div>
                    ...
                    <div style="margin-top:4px">${sustBadge}</div>
                  </div>
                </div>
              </td>
          ...
          // Fila de sólo lectura — solo mostrar badge si el campo existe en el pedido
          const hasSustFieldRO = 'sustitucion' in l;
          const sustBadgeRO = !hasSustFieldRO ? '' : l.sustitucion
              ...
              <td>
                <div style="display:flex;align-items:center;gap:10px">
                  ...
                  <div>
                    <div style="font-weight:600;font-size:.9rem">${l.name}</div>
                    <div style="font-size:.75rem;color:#888;text-transform:capitalize">${catLabel(l.category)} · ${l.unit}</div>
                    <div style="margin-top:4px">${sustBadgeRO}</div>
                  </div>
                </div>
              </td>
              <td style="text-align:center">
        ...
        <table class="order-products-table">
          ...
          <tfoot>
            <tr class="order-total-row">
              <td colspan="${isPending?4:3}" style="text-align:right;font-weight:700;padding-right:16px">
                Total del pedido:
              </td>
              <td style="text-align:right;font-size:1.15rem;font-weight:800;color:var(--green);white-space:nowrap" id="opd-total-cell">
...
function orderLineQty(lineIdx, delta) {
  ...
  if (minBtn) minBtn.innerHTML = `<i class="fas ${newQty === 1 ? 'fa-trash' : 'fa-minus'}"></i>`;
    ...
    return `
      <div class="no-prod-line" id="no-line-${idx}">
        ...
        <div class="no-qty-ctrl">
          <button class="no-qty-btn" onclick="noLineQty(${idx},-1)" title="${isOne?'Eliminar':'Disminuir'}">
            <i class="fas ${isOne ? 'fa-trash' : 'fa-minus'}"></i>
          </button>
...
function renderInventory() {
  ...
  // KPIs globales
  document.getElementById('invTotal').textContent = adminProducts.reduce((s, p) => s + p.stock, 0);
  document.getElementById('invLow').textContent   = adminProducts.filter(p => p.stock > 0 && p.stock < 20).length;
  document.getElementById('invZero').textContent  = adminProducts.filter(p => p.stock === 0).length;
  ...
  list.forEach(p => {
    ...
    tr.innerHTML = `
      <td>
        <div style="display:flex;align-items:center;gap:10px">
          <img src="${p.image}" style="width:36px;height:36px;border-radius:6px;object-fit:cover;flex-shrink:0" onerror="this.src='images/frutas.jpg'">
  ...
  _renderPaginator('invPaginator', pg, total, 'inventory', 'inventoryTable');
}
...
function catLabel(cat) {
  const m = { frutas:'Frutas', vegetales:'Vegetales', carnes:'Carnes', lacteos:'Lácteos', panaderia:'Panadería', mariscos:'Mariscos', bebidas:'Bebidas', despensa:'Despensa' };
  return m[cat] || cat;
}
